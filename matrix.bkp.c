...
int main() {
  matrix * A, * Ac, * B, * c, * d, * M, * ct, * mdp, *tmp;
  double dp;
  ...
  printf("\nDot product c' * d (1): %.2f\n", dp);

  dotProduct(c, d, &dp);
  printf("\nDot product c * d (2): %.2f\n", dp);
  ...
  setElement(c, 1, 1, 5.0);
  setElement(c, 2, 1, 8.0);
  setElement(c, 3, 1, 7.0);
  printf("\nc:\n");
  printMatrix(c);
